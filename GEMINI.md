...
- **Architecture:** Multi-Agent System using **LangGraph**, **CrewAI** & **Tavily Web Search**.
...
- **Key Constraints:**
  ...
  - **Search First Strategy:** Always attempt Web Search (Tavily) before generating new content via LLM.
  ...
  - **Caching:** Always check SQLite for existing recipes before querying Search or LLM.
...
4. **Web Search:** If not in DB -> Search Tavily using strict ingredients.
    - *Found:* Return parsed recipe.
    - *Not Found:* Proceed to Generation.
5. **Generate:** If search fails -> Agents attempt to build recipe via LLM.
...
6. **Save:** Store approved result in SQLite.
