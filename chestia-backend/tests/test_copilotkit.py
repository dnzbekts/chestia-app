...
from copilotkit_agent import (
    CopilotKitState,
    create_copilotkit_graph,
    get_initial_state,
    search_cache_node,
...
from config import COPILOTKIT_AGENT_NAME, COPILOTKIT_AGENT_DESCRIPTION
...
class TestCopilotKitState:
    ...
    def test_get_initial_state_filters_defaults(self):
        ...
        # "salt" and "water" are default ingredients
        state = get_initial_state(
            ingredients=["chicken", "salt", "water", "onion", "rice"],
            difficulty="intermediate",
            lang="tr"
        )
        
        # Default ingredients should be filtered out
        assert "salt" not in state["ingredients"]
        assert "water" not in state["ingredients"]
        assert "chicken" in state["ingredients"]
        assert "onion" in state["ingredients"]
    
    def test_get_initial_state_with_custom_message(self):
        """Initial state should use custom message if provided."""
        state = get_initial_state(
            ingredients=["beef", "potato"],
            difficulty="hard",
            lang="en",
            message="Make me a gourmet steak"
        )
...
class TestCopilotKitGraph:
    ...
    def test_should_continue_with_recipe(self):
    ...
    def test_should_continue_needs_generation(self):
...
class TestSearchCacheNode:
    ...
    def test_cache_hit(self, mock_db):
        ...
        result = search_cache_node(state)
    ...
    def test_cache_miss(self, mock_db):
        ...
        mock_conn = MagicMock()
        mock_db.get_db_connection.return_value = mock_conn
        mock_db.find_recipe_by_ingredients.return_value = None
...
class TestGenerateRecipeNode:
    ...
    @patch('copilotkit_agent.RecipeAgent')
    def test_successful_generation(self, mock_agent_class):
        ...
        result = generate_recipe_node(state)
    ...
    def test_generation_error(self, mock_agent_class):
        ...
        mock_agent = MagicMock()
        mock_agent.generate.return_value = {"error": "LLM failed"}
        mock_agent_class.return_value = mock_agent
...
class TestReviewRecipeNode:
    ...
    @patch('copilotkit_agent.ReviewAgent')
    def test_valid_recipe(self, mock_agent_class):
        ...
        result = review_recipe_node(state)
